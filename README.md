...
- constructor(selector: string, events: IEvents, content: HTMLElement) Конструктор принимает селектор, по которому в разметке страницы будет идентифицировано модальное окно, экземпляр класса `EventEmmitter` для возможности инициации событий, DOM-элемент с контентом.
...
- constructor(selector: string, events: IEvents, handleSubmit: Function)
